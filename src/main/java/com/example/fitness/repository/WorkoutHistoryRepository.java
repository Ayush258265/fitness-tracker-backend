...
public interface WorkoutHistoryRepository extends JpaRepository<WorkoutHistory, Long> {
	...
	@Query("SELECT w.workoutDate, SUM(w.caloriesBurned), SUM(w.duration), COUNT(w.id) " + "FROM WorkoutHistory w "
			+ "WHERE w.userId = :userId " + "AND w.workoutDate BETWEEN :startDate AND :endDate "
			+ "GROUP BY w.workoutDate")
	List<Object[]> getWeeklySummaryData(@Param("userId") Long userId, @Param("startDate") LocalDate startDate,
	...
	@Query("SELECT COALESCE(SUM(w.caloriesBurned), 0), COALESCE(SUM(w.duration), 0), COUNT(DISTINCT w.workoutDate) "
			+ "FROM WorkoutHistory w " + "WHERE w.userId = :userId "
			+ "AND w.workoutDate BETWEEN :startDate AND :endDate")
	Object[] getWeeklyTotals(@Param("userId") Long userId, @Param("startDate") LocalDate startDate,
			@Param("endDate") LocalDate endDate);
